...
$$.processLogin_anon = {"":
 ...
 super: "Closure",
 call$1: function(e) {
  return $._onLoadEnd(this.request_0);
}
};
...
$._onLoadEnd = function(request) {
  if ($.eqB(request.get$readyState(), 4) && $.eqB(request.get$status(), 200))
    ;
};
...
$.processLogin = function() {
  var url = $.S($.couchdb) + '/_session';
  var data = 'name=' + $.S($.getUsername()) + '&password=' + $.S($.getPassword());
  var request = $._XMLHttpRequestFactoryProvider_XMLHttpRequest();
  $.add$1(request.get$on().get$loadEnd(), new $.processLogin_anon(request));
  request.open$3$async('POST', url, true);
  request.setRequestHeader$2('X-CouchDB-WWW-Authenticate', 'Cookie');
  request.setRequestHeader$2('Content-Type', 'application/x-www-form-urlencoded');
  request.setRequestHeader$2('Accept', 'application/json');
  request.send$1(data);
};
...
$.getUsername = function() {
  return $.trim($.query('#username').get$value());
};
...
$.drawLoginLink = function() {
  $.add$1($.query('#loginbutton').get$on().get$click(), new $.drawLoginLink_anon());
};
...
$.getPassword = function() {
  return $.trim($.query('#password').get$value());
};
...
$.couchdb = 'http://127.0.0.1/couchdb';
...
$.$defineNativeClass('XMLHttpRequest', ["status?", "readyState?"], {
